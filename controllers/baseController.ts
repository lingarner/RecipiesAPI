...
export async function getAll(req: Request, res: Response): Promise<void> {
  ...
  export async function deleteSingle(req: Request, res: Response): Promise<void> {
    try {
      if (!ObjectId.isValid(req.params.id)) {
        res.status(400).json({ error: 'Must use a valid id to delete a recipe.' });
        return;
      }
...
export async function editSingle(req: Request, res: Response): Promise<void> {
    try {
      if (!ObjectId.isValid(req.params.id)) {
        res.status(400).json({ error: 'Must use a valid id to edit a recipe.' });
        return;
      }
      ...
      const result = await collection.replaceOne({ _id: id }, recipe);
